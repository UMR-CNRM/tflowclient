...
class SmsRcReader(object):
    ...
    def __init__(self, rc_file: str = None):
        ...
        if rc_file:
            ...
            if not (stat.S_ISREG(rc_stats.st_mode)
                    and (rc_stats.st_mode & (stat.S_IRWXG | stat.S_IRWXO)) == 0):
                raise SmsRcPermissionsError('{:s} must be a regular file with permission 0o600.'
                                            .format(rc_file))
    ...
    def get_password(self, host: str, user: str):
        ...
        raise KeyError('No credentials found for host={:s} and user={:s}.'
                       .format(host, user))
                ...
                if len(last_matches) == 0:
                    ...
                    if not from_suite:
                        ...
                        if s_name[0] != self.suite:
                            raise ValueError("The output's suite name does not match: {:s} vs {:s}"
...
class CdpInterface(FlowInterface, CdpOutputParserMixin):
    ...
    def credentials_summary(self) -> str:
        return '{:s}@{:s}'.format(self.credentials['user'],
    ...
    def _run_cdp_command(self,
                         command: str,
                         paths: typing.List[str]) -> typing.Tuple[str, bool]:
        todo = '\n'.join([command.format(p) for p in paths] + ['exit'])
        todo = todo.encode(encoding='utf-8')
        cmd = [self.credentials['cdp_path'],
               self.credentials['host'],
        ...
        rc = 0
        try:
            output = subprocess.check_output(cmd, input=todo,
                                             stderr=subprocess.PIPE)
        except subprocess.CalledProcessError as e:
            logger.error('Error while running cdp:\n  cmd=%s\n  exception=%s',
                         displayable_cmd, str(e))
            rc = e.returncode
            output = ('Error while running cdp: ' + str(e) + '\n' +
                      'Standard Output:\n' +
