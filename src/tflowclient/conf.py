...
class TFlowClientConfig(object):
    ...
    def logging_config(self, filename: str = None, level: str = None):
        ...
        # Handler
        if filename is None:
            filename = self._conf.get('logging', 'filename', fallback='~/.tflowclient.log')
            filename = os.path.expanduser(filename)
        f_handler = logging.handlers.TimedRotatingFileHandler(filename,
                                                              when='midnight',
                                                              interval=1,
                                                              backupCount=10,
                                                              encoding='utf-8')
        ...
        if level is None:
            m_logger.setLevel(self._conf.get('logging', 'level', fallback=logging.CRITICAL))
        else:
    ...
    def palette(self) -> typing.Union[typing.List[typing.Tuple], None]:
        ...
        if self._conf.has_section('palette'):
            ...
            for k, v in self._conf.items('palette'):
                ...
                v_items = [tuple(s.split('+')) if '+' in s else s
                           for s in v_items]
