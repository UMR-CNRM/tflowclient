...
class TestConf(unittest.TestCase):
    ...
    _default_palette = [('ABORTED', 'black', 'dark red'),
    ...
    def test_palette(self):
        self.assertListEqual(TFlowClientConfig(conf_txt="").palette,
                             self._default_palette)
        self.assertPaletteConf("treeline =  yellow,  dark green",
                               ('treeline', 'yellow', 'dark green'))
        self.assertPaletteConf("treeline=yellow,dark blue",
                               ('treeline', 'yellow', 'dark blue'))
        self.assertPaletteConf("treeline=yellow,dark blue,  bold+strikeout",
                               ('treeline', 'yellow', 'dark blue', ('bold', 'strikeout')))
        self.assertPaletteConf("ABORTED_f=yellow,dark blue,  bold+strikeout",
    ...
    def test_urwid_stuff(self):
        ...
        self.assertEqual(TFlowClientConfig(
            conf_txt="""[urwid]
            backend= curses
            """
        ).urwid_backend, 'curses')
        self.assertEqual(TFlowClientConfig(
            conf_txt="""[urwid]
            terminal_colors= 256
            """
        ).terminal_properties, dict(colors=256, has_underline=None, bright_is_bold=None))
        self.assertEqual(TFlowClientConfig(
            conf_txt="""[urwid]
            terminal_has_underline= False
            terminal_bright_is_bold=  none
            """
        ).terminal_properties, dict(colors=None, has_underline=False, bright_is_bold=None))
        self.assertEqual(TFlowClientConfig(
            conf_txt="""[urwid]
            terminal_has_underline= 1
            terminal_bright_is_bold=  0
            """
        ).terminal_properties, dict(colors=None, has_underline=True, bright_is_bold=False))
    ...
    def test_cdp_stuff(self):
        ...
        t_flow_conf = TFlowClientConfig(conf_txt="""[cdp]
        path=toto
        suite=groucho
        user=groucho
        host=server
